...
class BaseReporter {
  ...
  _registerSummary() {
    this._inspector.on('end', () => {
      const numFiles = this._inspector.numFiles;
      const files = (numFiles > 1) ? 'files' : 'file';
      const matches = (this._found > 1) ? 'matches' : 'match';
      let summary;
  ...
  _getFormattedLocation(instance) {
    const filePath = this._getRelativePath(instance.filename);
