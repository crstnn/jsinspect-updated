...
class NodeUtils {
   ...
   * with ESTree/Babel/parser spec ASTs.
   *
  ...
  static getChildren(node) {
    ...
    // by babel/parser that contain only newlines and spaces
    const filterIgnored = (nodes) => nodes.filter(node => {
  ...
  /**
   * Returns whether the first node appears before the second, by
  ...
  /**
   * Returns whether the nodes are part of an ES6 module import.
   *
   * @param   {Node[]}  nodes
   * @returns {boolean}
   */
  ...
  /**
   * Returns whether the nodes are part of an AMD require or define
   * expression.
   *
   * @param   {Node[]}  nodes
   * @returns {boolean}
   */
  static isAMD(nodes) {
    const hasAMDName = function (node) {
    ...
    for (let i = nodes.length - 1; i >= nodes.length - 5; i--) {
      ...
      const callee = nodes[i].expression.callee;
  ...
  /**
   * Returns whether the nodes are part of a CommonJS require statement.
   *
   * @param   {Node[]}  nodes
   * @returns {boolean}
   */
  static isCommonJS(nodes) {
    if (!nodes[0]) {
      ...
      for (let j = 0; j < nodes[0].declarations.length; j++) {
        const declaration = nodes[0].declarations[j];
  ...
  /**
   * Returns whether all nodes are of the same type.
   *
   * @param   {Node[]}  nodes
   * @returns {boolean}
   */
  ...
  /**
   * Returns whether all nodes have the same identifier.
   *
   * @param   {Node[]}  nodes
   * @returns {boolean}
   */
  ...
  /**
   * Returns whether all nodes have the same literal value.
   *
   * @param   {Node[]}  nodes
   * @returns {boolean}
   */
